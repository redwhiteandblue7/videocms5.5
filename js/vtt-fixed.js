 ...
 * @license MIT
 * 
 * This is a fixed version of the original videojs-vtt-thumbnails plugin
 * Fixes: 
 * Stopped thumbnail disappearing when moving mouse quickly
 * thumbnail not moving in sync with mouse esp. in FF (removed use of the style transform property)
 * Thumbnail not in sync with video (not taking account of progress bar holder margins)
 * Time cues from vtt file were being stored as int (whole seconds) instead of float (seconds with decimal) meaning many cues were being skipped!!!
 * Re-enabled the baseUrl option (removed in original plugin) as script didn't seem to handle running on a local server using IP address
 * 
 */
...
var vttThumbnailsPlugin = function () {
  ...
  vttThumbnailsPlugin.prototype.getBaseUrl = function getBaseUrl() {
    return this.options.baseUrl || this.player.options().baseUrl || '';
//    return [window.location.protocol, '//', window.location.hostname, window.location.port ? ':' + window.location.port : '', window.location.pathname].join('').split(/([^\/]*)$/gi).shift();
  ...
  vttThumbnailsPlugin.prototype.setupThumbnailElement = function setupThumbnailElement(data) {
    ...
    //added by me to fix the thumbnail not matching the video precisely especially at the start and end
    var holder = this.player.$('.vjs-progress-holder');
    var style = holder.currentStyle || window.getComputedStyle(holder);
    this.marginLeft = parseInt(style.marginLeft, 10);
    this.marginRight = parseInt(style.marginRight, 10);
  };
  ...
  vttThumbnailsPlugin.prototype.getXCoord = function getXCoord(bar, mouseX) {
    ...
    var x = mouseX - (rect.left + (window.pageXOffset || docEl.scrollLeft || 0));
    return x - this.marginLeft;
  };

  vttThumbnailsPlugin.prototype.onBarMousemove = function onBarMousemove(event) {
    //added by me to account for the width of the progress bar not being the same as the width of the progress control due to the margins
    this.updateThumbnailStyle(this.getXCoord(this.progressBar, event.clientX), this.progressBar.offsetWidth - this.marginLeft - this.marginRight);
  };
  ...
  vttThumbnailsPlugin.prototype.updateThumbnailStyle = function updateThumbnailStyle(x, width) {
    ...
    if (!currentStyle) {
        return;
        //removed this call on return as it was causing the thumbnail to disappear when the mouse was moved quickly, not sure why
//      return this.hideThumbnailHolder();
    }

        //replaced the below code, really not needed and caused stuttering on FF when moving the mouse at any speed
...
//    this.thumbnailHolder.style.marginLeft = '-' + parseInt(currentStyle.width) / 2 + 'px';
    var xPos = (1 - (width - x) / width) * width - parseInt(currentStyle.width) / 2 + this.marginLeft;
...
//    }
  ...
  vttThumbnailsPlugin.prototype.getSecondsFromTimestamp = function getSecondsFromTimestamp(timestamp) {
    ...
    //parseInt here caused the times to be stored in whole seconds, which meant several frames were skipped if you built the sprite image at more than 1 fps
